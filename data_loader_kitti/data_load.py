...
def generate_rand_rotm(x_lim=0, y_lim=0.349, z_lim=0):
...
class KITTI(Dataset):
    def __init__(self):
        ...
        self.point_clouds_dir =  r'C:\Users\praop\OneDrive\Desktop\NYU\AI4CE\code\DeepMapping-master\data_loader_kitti\kitti_point_clouds_voxel_sequence01_0.5.npy'
        ...
        for i in range(point_clouds.shape[0] - 1):

            source_pc = point_clouds[i+1]
            template_pc = point_clouds[i]

            if self.rand_aug == True:
                # Augmentation with random rotation
                R_rand = generate_rand_rotm()
                st_point_clouds.append([apply_rotation(source_pc, R_rand), template_pc])
            else:
